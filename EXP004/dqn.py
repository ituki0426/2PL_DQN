...
Device: use CUDA when available, otherwise CPU. NumPy simulation and replay storage stay on
CPU; network inference and gradient updates run on the selected device in FP64.
Network parameters, features, rewards and floating-point replay data use float64.
"""
...
class QNet(nn.Module):
    def __init__(self, n_in, h1, h2, n_items):
        ...
        self.net = nn.Sequential(
            nn.Linear(n_in, h1, dtype=torch.float64), nn.ReLU(),
            nn.Linear(h1, h2, dtype=torch.float64), nn.ReLU(),
            nn.Linear(h2, n_items, dtype=torch.float64),
        )
...
class DQNAgent:
    ...
    def greedy_from_features(self, feats, mask):
        with torch.no_grad():
            qv = self.q(torch.from_numpy(feats).to(device=self.device, dtype=self.dtype))
    ...
    def train(self, log=print):
        ...
        B = cfg.buffer_size
        buf_s = np.zeros((B, self.n_in), np.float64)
        buf_a = np.zeros(B, np.int64)
        buf_r = np.zeros(B, np.float64)
        buf_s2 = np.zeros((B, self.n_in), np.float64)
        buf_done = np.zeros(B, np.float64)
    ...
    def _update(self, rng, n_stored, buf_s, buf_a, buf_r, buf_s2, buf_done, buf_mask2):
        ...
        s = torch.from_numpy(buf_s[idx]).to(device=self.device, dtype=self.dtype)
        a = torch.from_numpy(buf_a[idx]).to(self.device)
        r = torch.from_numpy(buf_r[idx]).to(device=self.device, dtype=self.dtype)
        s2 = torch.from_numpy(buf_s2[idx]).to(device=self.device, dtype=self.dtype)
        done = torch.from_numpy(buf_done[idx]).to(device=self.device, dtype=self.dtype)
