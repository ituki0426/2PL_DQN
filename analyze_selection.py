...
Outputs (results/selection/): selection_by_step.csv, extreme_patterns.csv, reward_scale.csv,
fig_selection.pdf (copied to ../Paper/), log.txt (stdout)
"""
import argparse
import shutil
...
from catrl.dqn import DQNAgent, DQNConfig
from catrl.irt import info, posterior
from catrl.rules import MEPVPolicy, MFIPolicy
from catrl.scenarios import make_scenario
from catrl.simulate import rmse_by_step, run_cat
...
def by_step(df):
    ...
    for (m, t), d in df.groupby(["method", "step"]):
        ...
        out.append(dict(
            ...
            slope_b_on_prev=np.polyfit(d.theta_hat_prev, d.b, 1)[0],
...
def main():
    ...
    if out == Path("results/selection"):  # the manuscript figure (not for --quick runs to other folders)
        shutil.copy(out / "fig_selection.pdf", Path("../Paper/fig_selection.pdf"))
