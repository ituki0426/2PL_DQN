...
Every number printed here is read from results/<grid>/ files (mean.csv written by
`run_grid.py --aggregate`, the per-replication rep*.csv, and results/cost/cost.csv); the
...
def load(grid):
    agg = pd.read_csv(f"results/{grid}/mean.csv")
    reps = pd.concat([pd.read_csv(f) for f in sorted(glob.glob(f"results/{grid}/rep*.csv"))])
    return agg, reps
...
def cost_table():
    df = pd.read_csv("results/cost/cost.csv")
...
if __name__ == "__main__":
    {"main": main_table, "sensitivity": sensitivity_table, "state": state_table,
     "ablation": ablation_table, "guess": guess_table, "cost": cost_table}[sys.argv[1]]()
