...
depend on the weights. Outputs results/cost/cost.csv.
"""
import os
import time
...
from catrl.irt import make_bank, mle
from catrl.rules import MEPVPolicy, MFIPolicy, WeightedInfoPolicy
from catrl.simulate import prob
from run_grid import PROPOSED

torch.set_num_threads(1)
OUT = Path("results/cost")
OUT.mkdir(parents=True, exist_ok=True)


def history(bank, theta, t, rng):
    """t administered items (random) and responses, plus the MLE."""
    n = len(theta)
    ...
    th = mle(bank, items, resp, np.zeros(n))
