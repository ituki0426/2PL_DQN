...
既存の識別力スクリーニング（通常は `a <= 0` を除外）後、困難度 `b` が小さい15項目と
大きい15項目を各データセットの項目バンクから除外する。困難度が同じ場合は項目ID順で
順位を確定し、各側ちょうど15項目を除外する。回答行列、項目バンク、出力層とマスクの
列順を統一する。
...
|---|---:|---:|---:|---:|---:|
| 2019_2 | 3910 | 559 | 1117 | 260 | 41 |
| 2020_1 | 3772 | 539 | 1077 | 264 | 40 |
| 2021_2 | 4610 | 658 | 1317 | 224 | 41 |
...
上下15項目の除外前に作成した結果も設定が異なるため、再実行には別の `--out` を指定する。
...
このスクリーニング後の項目から同じく困難度の上下15項目を除外する。
出力は `<dataset>/strict/main/` に分離する。主実験の識別力条件は正値とする。
...
実データには、正の識別力でも困難度が極端な項目があるため、上下15項目を除外する。
除外後もDodd推定値の追加クリップは行わない。
全問正答・全問不正答者の推定値は、除外後のバンク内の困難度最大値・最小値へ近づく。
