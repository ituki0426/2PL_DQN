# EXP002：MPS 対応・MLE＋Dodd による実験

更新日：2026-09-10。現在の実装仕様をまとめた文書です。設定値の正本はこのディレクトリ内のコードです。

## 目的と他の実験との違い

EXP001 を基に、DQN の学習・推論を MPS 対応にした実験です。MPS が利用できる場合は自動で使用し、利用できない場合は CPU に切り替えます。能力推定法と学習条件は EXP001 と共通です。

EXP001 との主な違いは DQN の実行デバイスです。EXP003 は EXP001 を基にした CPU・EAP 版であり、本実験と比較すると能力推定法とデバイスの両方が異なります。
...
MPS に配置するのは Q ネットワーク・ターゲットネットワークと演算用テンソルです。NumPy による能力推定、反応生成、リプレイバッファの保持、解析的な規則の計算は CPU で行います。選択した項目番号は NumPy 配列として CPU に戻します。
...
# main の反復4を学習・評価
uv run python run_grid.py --experiment EXP002 --grid main --rep 4

# 保存済みの全反復を集計し、画像を生成
uv run python run_grid.py --experiment EXP002 --grid main --aggregate

# 推測ありの条件
uv run python run_grid.py --experiment EXP002 --grid guess --rep 4

# 動作確認用の結果は別のルートへ保存
uv run python run_grid.py --experiment EXP002 --grid main --rep 4 --quick --out /tmp/2pl-dqn-smoke

# 集計済みの結果から表を標準出力へ生成
uv run python make_tables.py --experiment EXP002 main

# 選択項目の分析と推論時間の測定
uv run python analyze_selection.py --experiment EXP002
uv run python benchmark_cost.py --experiment EXP002
```
...
標準保存先はリポジトリ内の `result/EXP002/` です。
...
`--out /tmp/2pl-dqn-smoke` の場合は `/tmp/2pl-dqn-smoke/EXP002/<grid>/` に保存します。集計・表生成時にも同じ `--out` を指定します。
