...
and the proposed one, selectable through DQNConfig. All conditions use EAP ability
estimates under N(0, 1) on the 81-point grid [-4, 4], and run on CPU.
...
Reward (cfg.reward):
  ...
  fi_ref             information at the reference ability, i.e. the EAP from the examinee's
...
from .irt import info, posterior, prob
...
class DQNAgent:
    ...
    def train(self, log=print):
        ...
        val_seed = cfg.seed * 7 + 12  # response stream inside run_cat is [seed, 1]
        empty = np.empty((1, 0), dtype=np.int64)
        prior_log_var = float(np.log(posterior(self.bank, empty, empty)[1][0]))
        ...
        for batch_idx in range(n_batches):
            ...
            rng.uniform(-0.5, 0.5, size=cfg.n_env)  # preserve EXP001 response/exploration random streams
            theta_hat = np.zeros(cfg.n_env)  # prior mean
            log_var = np.full(cfg.n_env, prior_log_var)
            episode = []
    ...
    def episode_return(self, items, resp, hist, theta0):
        ...
        ref = hist[-1]  # reference ability: EAP from the complete record of the episode
        if kind == "fi_ref":
